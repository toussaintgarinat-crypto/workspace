...
start-oria:
	...
	MATRIX_REGISTRATION_SHARED_SECRET=$$(grep '^ORIA_MATRIX_REGISTRATION_SHARED_SECRET=' $(ENV_FILE) 2>/dev/null | cut -d= -f2- || echo oria_local_dev_only_change_in_prod) \
	MATRIX_REGISTRATION_DISABLED=$$(grep '^ORIA_MATRIX_REGISTRATION_DISABLED=' $(ENV_FILE) 2>/dev/null | cut -d= -f2- || echo true) \
