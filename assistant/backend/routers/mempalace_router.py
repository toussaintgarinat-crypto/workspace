...
async def mempalace_wings(user: dict = Depends(get_current_user)):
    r = await mp_get(user, "/v1/api/wings")
    return r.json()
...
async def mempalace_search(
    ...
    r = await mp_post(user, "/v1/api/search", payload, timeout=15)
    return r.json()
...
async def mempalace_entries(
    ...
    r = await mp_get(user, f"/v1/api/wings/{wing}/drawers", params={"limit": limit})
    return r.json()
...
async def mempalace_export(
    ...
    r = await mp_get(user, "/v1/api/export", params={"format": format}, timeout=30)
