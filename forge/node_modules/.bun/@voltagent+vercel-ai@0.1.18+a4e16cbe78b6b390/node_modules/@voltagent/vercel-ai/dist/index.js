...
function mapToStreamPart(part) {
  return (0, import_ts_pattern.match)(part).returnType().with({ type: "text-delta" }, (p) => ({
    ...
    usage: (0, import_ts_pattern.match)(p).with(
      {
        usage: {
          promptTokens: import_ts_pattern.P.number,
          completionTokens: import_ts_pattern.P.number,
          totalTokens: import_ts_pattern.P.number
        }
      },
      (p2) => ({
        promptTokens: p2.usage.promptTokens,
        completionTokens: p2.usage.completionTokens,
        totalTokens: p2.usage.totalTokens
      })
    ).otherwise(() => void 0)
  })).with({ type: "error" }, (p) => ({
    type: "error",
...
function getUsageInfo(usage) {
  return (0, import_ts_pattern2.match)(usage).with({ promptTokens: import_ts_pattern2.P.number, completionTokens: import_ts_pattern2.P.number, totalTokens: import_ts_pattern2.P.number }, (u) => ({
