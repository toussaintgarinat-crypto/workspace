...
import { deepClone as deepClone2 } from "@voltagent/internal/utils";
...
var WorkflowRegistry = class _WorkflowRegistry extends EventEmitter {
  static {
...
// src/events/workflow-emitter.ts
var WorkflowEventEmitter = class _WorkflowEventEmitter extends EventEmitter2 {
  static {
...
// src/events/index.ts
var AgentEventEmitter = class _AgentEventEmitter extends EventEmitter3 {
  static {
  ...
  publishTimelineEventAsync(params) {
    ...
    this.timelineEventQueue.enqueue({
      ...
      operation: /* @__PURE__ */ __name(async () => {
        const clonedEvent = deepClone2(event);
...
var ConsoleLogger = class _ConsoleLogger {
  ...
  formatMessage(level, msg, obj) {
    ...
    const contextStr = Object.keys(this.context).length > 0 ? ` ${safeStringify(this.context)}` : "";
...
var InMemoryLogBuffer = class extends EventEmitter4 {
  static {
...
function matchStep(stepOrAgent) {
  return match(stepOrAgent).with({ type: "agent" }, (agentStep) => agentStep).with({ type: "func" }, (funcStep) => funcStep).with({ type: "conditional-when" }, (condStep) => condStep).with({ type: "parallel-all" }, (allStep) => allStep).with({ type: "parallel-race" }, (raceStep) => raceStep).otherwise(() => {
...
import { safeStringify as safeStringify4 } from "@voltagent/internal/utils";
...
var writeUpdateCache = /* @__PURE__ */ __name(async (projectPath, cache) => {
  try {
    ...
    const cacheFilePath = getCacheFilePath(projectPath);
    fs.writeFileSync(cacheFilePath, safeStringify2(cache, { indentation: 2 }), "utf8");
  } catch (error) {
    const logger2 = new LoggerProxy({ component: "update-cache" });
...
var getInstalledVersion = /* @__PURE__ */ __name(async (packageName, projectPath) => {
  try {
    const directPath = path2.join(projectPath, "node_modules", packageName, "package.json");
    if (fs2.existsSync(directPath)) {
      const content = fs2.readFileSync(directPath, "utf8");
      const pkg = JSON.parse(content);
      return pkg.version;
    }
    try {
      const resolvedPath = __require.resolve(`${packageName}/package.json`, {
        paths: [projectPath]
      });
    ...
    while (currentDir !== path2.dirname(currentDir)) {
      const modulePath = path2.join(currentDir, "node_modules", packageName, "package.json");
      if (fs2.existsSync(modulePath)) {
        const content = fs2.readFileSync(modulePath, "utf8");
        const pkg = JSON.parse(content);
        return pkg.version;
      }
...
var checkForUpdates = /* @__PURE__ */ __name(async (packagePath, options) => {
  try {
    const rootDir = packagePath ? path2.dirname(packagePath) : path2.resolve(process.cwd());
    const packageJsonPath = packagePath || path2.join(rootDir, "package.json");
    ...
    let packageJson;
    try {
      const packageJsonContent = fs2.readFileSync(packageJsonPath, "utf-8");
...
var updateAllPackages = /* @__PURE__ */ __name(async (packagePath) => {
  try {
    ...
    const rootDir = packagePath ? path2.dirname(packagePath) : process.cwd();
...
import { deepClone as deepClone3 } from "@voltagent/internal";
...
import { safeStringify as safeStringify3 } from "@voltagent/internal/utils";
var LibSQLWorkflowExtension = class {
  ...
  async storeWorkflowHistory(entry) {
    await this.client.execute({
      ...
      args: [
        ...
        safeStringify3(entry.input),
        entry.output ? safeStringify3(entry.output) : null,
        entry.userId || null,
        entry.conversationId || null,
        entry.metadata ? safeStringify3(entry.metadata) : null,
  ...
  async updateWorkflowHistory(id, updates) {
    ...
    if (updates.output !== void 0) {
      ...
      args.push(safeStringify3(updates.output));
    }
    ...
    if (updates.metadata !== void 0) {
      ...
      const metadataJson = safeStringify3(updates.metadata);
  ...
  async storeWorkflowStep(step) {
    await this.client.execute({
      ...
      args: [
        ...
        step.input ? safeStringify3(step.input) : null,
        step.output ? safeStringify3(step.output) : null,
        step.error ? safeStringify3(step.error) : null,
        step.agentExecutionId || null,
        step.parallelIndex || null,
        step.parallelParentStepId || null,
        step.metadata ? safeStringify3(step.metadata) : null,
  ...
  async updateWorkflowStep(id, updates) {
    ...
    if (updates.error !== void 0) {
      ...
      args.push(safeStringify3(updates.error));
    }
    ...
    if (updates.metadata !== void 0) {
      setClauses.push("metadata = ?");
      args.push(safeStringify3(updates.metadata));
    }
    setClauses.push("updated_at = ?");
    args.push((/* @__PURE__ */ new Date()).toISOString());
    args.push(id);
  ...
  async storeWorkflowTimelineEvent(event) {
    await this.client.execute({
      ...
      args: [
        ...
        event.status,
        event.level || "INFO",
        event.input ? safeStringify3(event.input) : null,
        event.output ? safeStringify3(event.output) : null,
        event.statusMessage ? safeStringify3(event.statusMessage) : null,
        event.metadata ? safeStringify3(event.metadata) : null,
        event.traceId || null,
...
var LibSQLStorage = class {
  ...
  async addMessage(message, conversationId = "default") {
    ...
    const contentString = safeStringify4(message.content);
  ...
  async addHistoryEntry(key, value, agentId) {
    ...
    try {
      ...
      const inputJSON = value.input ? safeStringify4(value.input) : null;
      const outputJSON = value.output ? safeStringify4(value.output) : null;
      const usageJSON = value.usage ? safeStringify4(value.usage) : null;
      const metadataJSON = value.metadata ? safeStringify4(value.metadata) : null;
      await this.client.execute({
  ...
  async addHistoryStep(key, value, historyId, agentId) {
    ...
    try {
      ...
      const serializedValue = safeStringify4(value);
      await this.client.execute({
  ...
  async addTimelineEvent(key, value, historyId, agentId) {
    ...
    try {
      ...
      const inputJSON = value.input ? safeStringify4(value.input) : null;
      const outputJSON = value.output ? safeStringify4(value.output) : null;
      const statusMessageJSON = value.statusMessage ? safeStringify4(value.statusMessage) : null;
      const metadataJSON = value.metadata ? safeStringify4(value.metadata) : null;
      const tagsJSON = value.tags ? safeStringify4(value.tags) : null;
      await this.client.execute({
        sql: `INSERT OR REPLACE INTO ${tableName} 
  ...
  async createConversation(conversation) {
    ...
    const metadataString = safeStringify4(conversation.metadata);
  ...
  async updateConversation(id, updates) {
    ...
    try {
      ...
      if (updates.metadata !== void 0) {
        ...
        args.push(safeStringify4(updates.metadata));
      }
    ...
    try {
      ...
      for (const row of oldFormatData.rows) {
        ...
        try {
          ...
          migratedIds.add(id);
          migratedCount++;
          const inputJSON = valueObj.input ? safeStringify4(valueObj.input) : null;
          const outputJSON = valueObj.output ? safeStringify4(valueObj.output) : null;
          const usageJSON = valueObj.usage ? safeStringify4(valueObj.usage) : null;
          await this.client.execute({
          ...
          if (Array.isArray(valueObj.events)) {
            for (const event of valueObj.events) {
              try {
                ...
                let inputData = null;
                if (event.input) {
                  inputData = safeStringify4({ input: event.input });
                } else if (event.data?.input) {
                  inputData = safeStringify4({ input: event.data.input });
                } else if (input) {
                  inputData = safeStringify4({ input });
                }
                input = "";
                let metadata = null;
                if (event.metadata) {
                  metadata = safeStringify4(event.metadata);
                } else if (event.data) {
                  metadata = safeStringify4({
                ...
                if (eventType === "agent") {
                  ...
                  await this.client.execute({
                    ...
                    args: [
                      ...
                      // tags
                      inputData,
                      event.data.output ? safeStringify4(event.data.output) : null,
                      eventName === "agent:error" ? safeStringify4(event.data.error) : null,
                      metadata
                    ]
                  });
                  ...
                  if (eventName === "memory:saveMessage") {
                    ...
                    await this.client.execute({
                      ...
                      args: [
                        ...
                        inputData,
                        event.data.output ? safeStringify4(event.data.output) : null,
                        event.error ? safeStringify4(event.error) : null,
                        safeStringify4({
                          id: "memory",
                          agentId: event.affectedNodeId?.split("_").pop()
                        })
                    ...
                    await this.client.execute({
                      ...
                      args: [
                        ...
                        // tags
                        inputData,
                        event.output ? safeStringify4(event.output) : null,
                        event.error ? safeStringify4(event.error) : null,
                        metadata
                      ]
                    });
...
// src/agent/agent.ts
import { safeStringify as safeStringify8 } from "@voltagent/internal/utils";
import { P as P2, match as match3 } from "ts-pattern";

// src/memory/in-memory/index.ts
import { deepClone as deepClone4 } from "@voltagent/internal/utils";
var InMemoryStorage = class {
  ...
  async getHistoryEntry(key) {
    ...
    return entry ? deepClone4(entry) : void 0;
  }
  /**
   * Get a history step by ID
   */
  async getHistoryStep(key) {
    ...
    return step ? deepClone4(step) : void 0;
  }
  /**
   * Add a history entry
   */
  ...
  async getAllHistoryEntriesByAgent(agentId, page, limit) {
    ...
    const sortedEntries = entries.map((entry) => deepClone4(entry)).sort((a, b) => {
  ...
  async getWorkflowHistory(id) {
    ...
    return entry ? deepClone4(entry) : null;
  }
  /**
  ...
  async getWorkflowHistoryByWorkflowId(workflowId) {
    ...
    return entries.map((entry) => deepClone4(entry)).sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime());
  }
  /**
   * Update a workflow history entry
   */
  ...
  async getWorkflowStep(id) {
    ...
    return step ? deepClone4(step) : null;
  }
  /**
  ...
  async getWorkflowSteps(workflowHistoryId) {
    ...
    return steps.map((step) => deepClone4(step)).sort((a, b) => a.stepIndex - b.stepIndex);
  }
  /**
  ...
  async getWorkflowTimelineEvent(id) {
    ...
    return event ? deepClone4(event) : null;
  }
  /**
  ...
  async getWorkflowTimelineEvents(workflowHistoryId) {
    ...
    return events.map((event) => deepClone4(event)).sort((a, b) => {
...
import { P, match as match2 } from "ts-pattern";
function transformStreamEventToStreamPart(event) {
  const baseStreamPart = match2(event).returnType().with({ type: "tool-call", data: P.not(P.nullish) }, (e) => ({
    type: "tool-call",
...
import { safeStringify as safeStringify5 } from "@voltagent/internal/utils";
var TelemetryServiceApiClient = class {
  ...
  async request(method, path4, body) {
    ...
    const response = await this.fetchImplementation(url, {
      method,
      headers,
      body: body ? safeStringify5(body) : void 0
    });
    if (!response.ok) {
      let errorBody;
      try {
      ...
      throw new Error(
        `API request failed: ${response.status} ${response.statusText} - ${safeStringify5(errorBody)}`
      );
    }
...
import { safeStringify as safeStringify6 } from "@voltagent/internal/utils";
...
function endOperationSpan(options, logger2) {
  ...
  try {
    const attributes = {};
    if (data.input) {
      attributes["ai.prompt.messages"] = typeof data.input === "string" ? data.input : safeStringify6(data.input);
    }
    if (data.output) {
      attributes["ai.response.text"] = typeof data.output === "string" ? data.output : safeStringify6(data.output);
    }
    ...
    if (data.metadata && typeof data.metadata === "object") {
      for (const [key, value] of Object.entries(data.metadata)) {
        if (value != null && typeof key === "string" && !key.startsWith("internal.")) {
          attributes[`metadata.${key}`] = typeof value === "string" || typeof value === "number" || typeof value === "boolean" ? value : safeStringify6(value);
        }
      }
    }
  ...
  const toolSpan = tracer.startSpan(
    `tool.execution:${toolName}`,
    {
      ...
      attributes: {
        ...
        "tool.name": toolName,
        "tool.arguments": toolInput ? safeStringify6(toolInput) : void 0,
        "agent.id": agentId
      }
    },
    parentOtelContext
...
function endToolSpan(options, logger2) {
  ...
  try {
    ...
    span.setAttribute("tool.result", safeStringify6(toolResultContent));
    if (isError) {
...
import { safeStringify as safeStringify7 } from "@voltagent/internal/utils";
...
Context: ${safeStringify7(context, { indentation: 2 })}`;
      }
      const taskMessage = {
        role: "user",
        content: taskContent
      ...
      if (method === "generateText") {
        ...
        finalResult = safeStringify7(response.object);
        ...
        finalResult = safeStringify7(finalObject);
        finalMessages = [taskMessage, { role: "assistant", content: finalResult }];
      } else {
...
${retrieverContext}`;
  ...
  addStepToHistory(step, context) {
    ...
    const finalStep = {
      ...step,
      ...match3(context).with({ parentAgentId: P2.not(P2.nullish) }, () => ({
        subAgentId: this.id,
  ...
  async generateObject(input, schema, options = {}) {
    ...
    try {
      ...
      this.publishTimelineEvent(operationContext, agentSuccessEvent);
      const responseStr = safeStringify8(response.object);
      this.addAgentEvent(operationContext, "finished", "completed", {
        output: responseStr,
        usage: response.usage,
        status: "completed",
  ...
  async streamObject(input, schema, options = {}) {
    ...
    const response = await this.llm.streamObject({
      ...
      onFinish: /* @__PURE__ */ __name(async (result) => {
        ...
        this.publishTimelineEvent(operationContext, agentSuccessEvent);
        const responseStr = safeStringify8(result.object);
        this.addAgentEvent(operationContext, "finished", "completed", {
          input: messages,
          output: responseStr,
          usage: result.usage,
...
var MCPClient = class extends EventEmitter5 {
  static {
...
import { convertJsonSchemaToZod as convertJsonSchemaToZod2 } from "zod-from-json-schema";

// src/server/api.routes.ts
import { createRoute, z as z6 } from "@hono/zod-openapi";
var ParamsSchema = z6.object({
...
var ErrorSchema = z6.object({
  ...
  error: z6.string().openapi({ description: "Error message" })
});
var SubAgentResponseSchema = z6.object({
  id: z6.string(),
  name: z6.string(),
  description: z6.string(),
  status: z6.string().openapi({ description: "Current status of the sub-agent" }),
  // Keeping string for now
  model: z6.string(),
  tools: z6.array(z6.any()).optional(),
  memory: z6.any().optional()
}).passthrough();
var AgentResponseSchema = z6.object({
  id: z6.string(),
  name: z6.string(),
  description: z6.string(),
  status: z6.string().openapi({ description: "Current status of the agent" }),
  // Reverted to z.string()
  model: z6.string(),
  tools: z6.array(z6.any()),
  // Simplified tool representation
  subAgents: z6.array(SubAgentResponseSchema).optional().openapi({ description: "List of sub-agents" }),
  // Use SubAgent schema
  memory: z6.any().optional(),
  // Simplified memory representation
  isTelemetryEnabled: z6.boolean().openapi({ description: "Indicates if telemetry is configured for the agent" })
  // Add other fields from getFullState if necessary and want them documented
}).passthrough();
var GenerateOptionsSchema = z6.object({
  userId: z6.string().optional().openapi({ description: "Optional user ID for context tracking" }),
  conversationId: z6.string().optional().openapi({
    description: "Optional conversation ID for context tracking"
  }),
  contextLimit: z6.number().int().positive().optional().default(10).openapi({
    description: "Optional limit for conversation history context"
  }),
  maxSteps: z6.number().int().positive().optional().openapi({
    description: "Maximum number of steps (turns) for this specific request (overrides agent's maxSteps)",
    example: 5
  }),
  temperature: z6.number().min(0).max(1).optional().default(0.7).openapi({ description: "Controls randomness (0-1)" }),
  maxTokens: z6.number().int().positive().optional().default(4e3).openapi({ description: "Maximum tokens to generate" }),
  topP: z6.number().min(0).max(1).optional().default(1).openapi({
    description: "Controls diversity via nucleus sampling (0-1)"
  }),
  frequencyPenalty: z6.number().min(0).max(2).optional().default(0).openapi({ description: "Penalizes repeated tokens (0-2)" }),
  presencePenalty: z6.number().min(0).max(2).optional().default(0).openapi({ description: "Penalizes tokens based on presence (0-2)" }),
  seed: z6.number().int().optional().openapi({ description: "Optional seed for reproducible results" }),
  stopSequences: z6.array(z6.string()).optional().openapi({ description: "Stop sequences to end generation" }),
  extraOptions: z6.record(z6.string(), z6.unknown()).optional().openapi({ description: "Provider-specific options" }),
  userContext: z6.record(z6.string(), z6.unknown()).optional().openapi({
...
var ContentPartSchema = z6.union([
  z6.object({
    // Text part
  ...
  z6.object({
    ...
    image: z6.string().openapi({ description: "Base64 encoded image data or a URL" }),
    mimeType: z6.string().optional().openapi({ example: "image/jpeg" }),
    alt: z6.string().optional().openapi({ description: "Alternative text for the image" })
  }).openapi({
    example: {
      type: "image",
  ...
  z6.object({
    ...
    data: z6.string().openapi({ description: "Base64 encoded file data" }),
    filename: z6.string().openapi({ example: "document.pdf" }),
    mimeType: z6.string().openapi({ example: "application/pdf" }),
    size: z6.number().optional().openapi({ description: "File size in bytes" })
  }).openapi({
    example: {
      type: "file",
      data: "...",
...
var MessageContentSchema = z6.union([
  z6.string().openapi({ description: "Plain text content" }),
  z6.array(ContentPartSchema).openapi({ description: "An array of content parts (text, image, file)." })
]);
var MessageObjectSchema = z6.object({
  role: z6.enum(["system", "user", "assistant", "tool"]).openapi({
...
var TextRequestSchema = z6.object({
  input: z6.union([
    z6.string().openapi({
      description: "Input text for the agent",
      example: "Tell me a joke!"
    }),
    z6.array(MessageObjectSchema).openapi({
...
var TextResponseSchema = z6.object({
  success: z6.literal(true),
  data: z6.string().openapi({ description: "Generated text response" })
  // Assuming simple text response for now
});
var StreamTextEventSchema = z6.object({
  text: z6.string().optional(),
  timestamp: z6.string().datetime().optional(),
  type: z6.enum(["text", "completion", "error"]).optional(),
  done: z6.boolean().optional(),
  error: z6.string().optional()
});
var BasicJsonSchema = z6.object({
  type: z6.literal("object"),
  properties: z6.record(
    z6.object({
      type: z6.enum(["string", "number", "boolean", "object", "array", "null", "any"])
    })
  ).optional().openapi({
  ...
  required: z6.array(z6.string()).optional().openapi({
...
var ObjectRequestSchema = z6.object({
  input: z6.union([
    z6.string().openapi({ description: "Input text prompt" }),
    z6.array(MessageObjectSchema).openapi({ description: "Conversation history" })
  ]),
...
var ObjectResponseSchema = z6.object({
  success: z6.literal(true),
  data: z6.object({}).passthrough().openapi({ description: "Generated object response" })
  // Using passthrough object
});
var StreamObjectEventSchema = z6.any().openapi({
    ...
    200: {
      content: {
        "application/json": {
          schema: z6.object({
            success: z6.literal(true),
            data: z6.array(AgentResponseSchema).openapi({ description: "List of registered agents" })
          })
        }
      },
...
var WorkflowResponseSchema = z6.object({
  id: z6.string().openapi({ description: "Unique workflow identifier" }),
  name: z6.string().openapi({ description: "Human-readable workflow name" }),
  purpose: z6.string().openapi({ description: "Description of what the workflow does" }),
  stepsCount: z6.number().int().openapi({ description: "Number of steps in the workflow" }),
  status: z6.enum(["idle", "running", "completed", "error"]).openapi({
    description: "Current status of the workflow"
  })
}).openapi({ description: "Workflow information" });
var getWorkflowsRoute = createRoute({
  method: "get",
  path: "/workflows",
  responses: {
    200: {
      content: {
        "application/json": {
          schema: z6.object({
            success: z6.literal(true),
            data: z6.array(WorkflowResponseSchema).openapi({ description: "List of registered workflows" })
          })
        }
      },
...
var WorkflowExecutionRequestSchema = z6.object({
  input: z6.any().openapi({
...
var WorkflowStreamEventSchema = z6.object({
  type: z6.string().openapi({ description: "Event type" }),
  executionId: z6.string().openapi({ description: "Workflow execution ID" }),
  from: z6.string().openapi({ description: "Source of the event" }),
  input: z6.any().optional(),
  output: z6.any().optional(),
  status: z6.enum(["pending", "running", "success", "error", "suspended"]),
  timestamp: z6.string(),
  stepIndex: z6.number().optional(),
  metadata: z6.record(z6.any()).optional(),
  error: z6.any().optional()
});
var streamWorkflowRoute = createRoute({
  method: "post",
...
var executeWorkflowRoute = createRoute({
  method: "post",
...
var WorkflowSuspendRequestSchema = z6.object({
  reason: z6.string().optional().openapi({ description: "Reason for suspension" })
}).openapi({ description: "Workflow suspension request" });
var WorkflowSuspendResponseSchema = z6.object({
  success: z6.literal(true),
  data: z6.object({
    executionId: z6.string(),
    status: z6.literal("suspended"),
    suspension: z6.object({
      suspendedAt: z6.string(),
      reason: z6.string().optional()
    })
  }).openapi({ description: "Workflow suspension result" })
}).openapi({ description: "Successful workflow suspension response" });
var suspendWorkflowRoute = createRoute({
  method: "post",
...
var WorkflowResumeResponseSchema = z6.object({
  success: z6.literal(true),
  data: z6.object({
    executionId: z6.string(),
    startAt: z6.string(),
    endAt: z6.string().optional(),
    status: z6.string(),
    result: z6.any()
  }).openapi({ description: "Workflow resume result" })
}).openapi({ description: "Successful workflow resume response" });
var resumeWorkflowRoute = createRoute({
  method: "post",
  ...
  request: {
    ...
    body: {
      content: {
        "application/json": {
          schema: z6.object({
            resumeData: z6.any().optional().openapi({
            ...
            options: z6.object({
              stepId: z6.string().optional().openapi({
...
var ErrorSchema2 = z7.object({
  success: z7.literal(false),
  error: z7.string().openapi({ description: "Error message" })
});
var LogEntrySchema = z7.object({
  timestamp: z7.string(),
  level: z7.enum(["trace", "debug", "info", "warn", "error", "fatal", "silent"]),
  msg: z7.string(),
  component: z7.string().optional(),
  agentId: z7.string().optional(),
  conversationId: z7.string().optional(),
  workflowId: z7.string().optional(),
  executionId: z7.string().optional(),
  userId: z7.string().optional(),
  error: z7.object({
    type: z7.string(),
    message: z7.string(),
    stack: z7.string().optional()
  }).optional()
}).catchall(z7.any());
var LogQuerySchema = z7.object({
  limit: z7.number().int().positive().max(1e3).optional().default(100).openapi({
    description: "Maximum number of log entries to return",
    example: 100
  }),
  level: z7.enum(["trace", "debug", "info", "warn", "error", "fatal"]).optional().openapi({
    description: "Minimum log level to filter by",
    example: "info"
  }),
  agentId: z7.string().optional().openapi({
    description: "Filter logs by agent ID",
    example: "agent-123"
  }),
  conversationId: z7.string().optional().openapi({
    description: "Filter logs by conversation ID",
    example: "conv-456"
  }),
  workflowId: z7.string().optional().openapi({
    description: "Filter logs by workflow ID",
    example: "workflow-789"
  }),
  executionId: z7.string().optional().openapi({
    description: "Filter logs by workflow execution ID",
    example: "exec-012"
  }),
  since: z7.string().datetime().optional().openapi({
    description: "Return logs since this timestamp (ISO 8601)",
    example: "2024-01-01T00:00:00Z"
  }),
  until: z7.string().datetime().optional().openapi({
    description: "Return logs until this timestamp (ISO 8601)",
    example: "2024-01-01T23:59:59Z"
  })
});
var LogEntriesResponseSchema = z7.object({
  success: z7.literal(true),
  data: z7.array(LogEntrySchema),
  total: z7.number().int(),
  query: LogQuerySchema
});
var getLogsRoute = createRoute2({
  method: "get",
  path: "/api/logs",
  request: {
    query: LogQuerySchema
...
var CustomEndpointSchema = z8.object({
  path: z8.string().startsWith("/"),
  method: z8.enum(["get", "post", "put", "patch", "delete", "options", "head"]),
  handler: z8.function().args(z8.any()).returns(z8.any()),
...
import { safeStringify as safeStringify9 } from "@voltagent/internal/utils";
  ...
  sendToClient(client, data) {
    try {
      if (client.ws.readyState === client.ws.OPEN) {
        client.ws.send(safeStringify9(data));
      }
    } catch (error) {
...
app.openapi(objectRoute, async (c) => {
  ...
  try {
    ...
    const schemaInZodObject = convertJsonSchemaToZod2(schema);
